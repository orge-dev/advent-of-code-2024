...
Here's a solution to Day 9 Part 1 first in Python, then I'll help translate it to Emacs Lisp:
...
Let me translate this to Emacs Lisp:


#+begin_src elisp
(defun parse-disk-map (input-str)
  "Convert string of numbers into list of integers."
  (mapcar #'string-to-number 
          (split-string input-str "" t)))

(defun expand-disk-map (numbers)
  "Convert compressed format into list showing file IDs and free space."
  (let ((result '())
        (file-id 0))
...
(defun compact-disk (expanded)
  "Move files to leftmost available space."
  (let ((result (copy-sequence expanded)))
    ;; Process from right to left
    (cl-loop for i from (1- (length result)) downto 0
             when (numberp (nth i result))
             do (cl-loop for j from 0 below (length result)
                        when (eq (nth j result) '?)
                        do (progn
                             (setf (nth j result) (nth i result))
                             (unless (= i j)
                               (setf (nth i result) '?))
                             (cl-return))))
    result))

(defun calculate-checksum (compacted)
  "Calculate checksum based on position * file ID."
  (cl-loop for file-id in compacted
           for pos from 0
           when (numberp file-id)
           sum (* pos file-id)))

(defun solve-part1 (input-str)
  (let* ((numbers (parse-disk-map input-str))
         (expanded (expand-disk-map numbers))
         (compacted (compact-disk expanded)))
    (calculate-checksum compacted)))

;; Test with example
(solve-part1 "2333133121414131402")  ; Should return 1928
